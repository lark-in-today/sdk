...
class Requests {
  static auth(res, url, params, data) {
    ...
    if (res.data.msg.match(/WARNING_000/)) {
      ...
      let ret = crypto.Ed25519.verify(crypto.decodeBase64(_tk), crypto.decodeBase64(stoken), crypto.decodeBase64(store.get('public_key')));
  ...
  static request(method, url, params, data) {
    ...
    return axios({
      ...
      params: params,
      data: data
    }).then(res => Requests.auth(res, url, params, data));
  }
